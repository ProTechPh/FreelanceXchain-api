...
2. Review the Hardhat configuration in hardhat.config.cjs, which defines:
   ...
   - Network configurations for hardhat, ganache, sepolia, polygon, and mumbai
...
The application will be available at http://localhost:3000. The src/app.ts file configures the Express server with middleware for security, CORS, request logging, and error handling, and mounts the API routes under the /api path.
