...
jest.unstable_mockModule(resolveModule('src/middleware/rate-limiter.ts'), () => ({
  ...
  walletRateLimiter: (req: any, res: any, next: any) => next(),
...
jest.unstable_mockModule(resolveModule('src/services/payment-service.ts'), () => ({
  ...
  getMyPayments: jest.fn(async () => ({ success: true, data: { items: [], total: 0, hasMore: false } })),
  getPaymentSummary: jest.fn(async () => ({ success: true, data: { totalEarnings: 0, totalSpent: 0, available: true } })),
