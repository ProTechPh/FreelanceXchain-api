...
describe('Auth Routes - Uncovered Phone/Email/Magic/Verify Routes', () => {
  ...
  jest.unstable_mockModule(resolveModule('src/utils/route-helpers.ts'), () => ({
    ...
    asyncHandler: (fn: any) => (req: any, res: any, next: any) => {
      Promise.resolve(fn(req, res, next)).catch(() => {
        res.status(500).json({ error: { code: 'INTERNAL_ERROR', message: 'An unexpected error occurred' }, timestamp: new Date().toISOString(), requestId: 'test-request-id' });
      });
    },
  ...
  describe('POST /login/verify-token', () => {
    ...
    it('should return 401 when verifyAuthToken returns AUTH_INVALID_TOKEN', async () => {
      mockVerifyAuthToken.mockResolvedValue({
