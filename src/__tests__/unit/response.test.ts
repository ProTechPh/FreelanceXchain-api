...
describe('response', () => {
  ...
  describe('sendValidationError', () => {
    it('responds with status 400 and VALIDATION_ERROR code', () => {
      const res = mockRes();
      sendValidationError(res, 'Missing required fields', 'rid-1');
      expect(res.status).toHaveBeenCalledWith(400);
      expect(res.json).toHaveBeenCalledWith({
    ...
    it('includes details when provided', () => {
      const res = mockRes();
      sendValidationError(res, 'Invalid input', 'rid-2', ['email required']);
      expect(res.json).toHaveBeenCalledWith({
        error: {
          code: 'VALIDATION_ERROR',
    ...
    it('omits details field when not provided', () => {
      const res = mockRes();
      sendValidationError(res, 'No details here', 'rid-3');
