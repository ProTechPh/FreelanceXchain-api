/**
 * Escrow Blockchain Integration Tests
...
describe('Escrow Blockchain Integration', () => {
  ...
  describe('approveMilestone', () => {
    it('should approve milestone and release payment', async () => {
      ...
      expect(mockContract.approveMilestone).toHaveBeenCalledWith(0);
    });
  });
  ...
  describe('disputeMilestone', () => {
    it('should dispute milestone', async () => {
      ...
      expect(mockContract.disputeMilestone).toHaveBeenCalledWith(0);
    });
  });
  ...
  describe('resolveDispute', () => {
    ...
    it('should resolve dispute in favor of employer', async () => {
      ...
      const { resolveDispute } = await import('../escrow-blockchain.js');
      const result = await resolveDispute('0xEscrowContract', 0, false);

      expect(mockContract.resolveDispute).toHaveBeenCalledWith(0, false);
    });
  });
  ...
  describe('refundMilestone', () => {
    it('should refund milestone', async () => {
      ...
      mockContract.refundMilestone.mockResolvedValue({
        wait: (jest.fn() as any).mockResolvedValue(mockReceipt),
      });

      const { refundMilestone } = await import('../escrow-blockchain.js');
      const result = await refundMilestone('0xEscrowContract', 0);

      expect(result).toEqual({
        transactionHash: '0xRefundHash',
        receipt: mockReceipt,
      });

      expect(mockContract.refundMilestone).toHaveBeenCalledWith(0);
    });
  });
  ...
  describe('cancelContract', () => {
    it('should cancel contract and refund remaining funds', async () => {
      ...
      mockContract.cancelContract.mockResolvedValue({
        wait: (jest.fn() as any).mockResolvedValue(mockReceipt),
      });

      const { cancelContract } = await import('../escrow-blockchain.js');
      const result = await cancelContract('0xEscrowContract');

      expect(result).toEqual({
        transactionHash: '0xCancelHash',
        receipt: mockReceipt,
      });

      expect(mockContract.cancelContract).toHaveBeenCalled();
    });
  });
  ...
  describe('getMilestoneCount', () => {
    it('should return milestone count', async () => {
      ...
      const count = await getMilestoneCount('0xEscrowContract');
  ...
  describe('getEscrowBalance', () => {
    it('should return escrow balance', async () => {
      ...
      const balance = await getEscrowBalance('0xEscrowContract');
  ...
  describe('getRemainingAmount', () => {
    it('should return remaining amount to be released', async () => {
      ...
      const remaining = await getRemainingAmount('0xEscrowContract');

      expect(remaining).toBe(BigInt('1500000000000000000'));
    });
  });
});
