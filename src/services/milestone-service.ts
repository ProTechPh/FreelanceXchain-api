...
export async function approveMilestone(
  ...
  try {
    ...
    if (contract.employer_id !== input.employerId) {
      return {
        success: false,
        error: { code: 'UNAUTHORIZED', message: 'You are not authorized to approve this milestone' },
      };
    }
    ...
    if (milestone.status !== 'submitted') {
      return {
        ...
        error: {
          code: 'INVALID_STATUS',
          message: `Cannot approve milestone with status "${milestone.status}"`,
        },
      };
    }
    ...
    // Get freelancer user details for email
    ...
    const notificationResult = await createNotification({
      ...
      message: `Your milestone "${milestone.title}" has been approved. Payment will be released shortly.`,
      data: {
        milestoneId: input.milestoneId,
        type: 'milestone',
      },
    ...
    if (freelancer?.email) {
      await sendMilestoneApprovedEmail(freelancer.email, {
        freelancerName: freelancer.full_name || 'Freelancer',
        milestoneTitle: milestone.title,
        amount: `$${milestone.amount.toFixed(2)}`,
        contractUrl: `${process.env['FRONTEND_URL'] || 'http://localhost:3000'}/contracts/${milestone.contractId}`,
      });
    }
