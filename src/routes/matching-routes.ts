 ...
 */
router.get('/projects', authMiddleware, requireKyc, async (req: Request, res: Response) => {
 ...
 */
router.get('/freelancers/:projectId', authMiddleware, requireKyc, validateUUID(['projectId']), async (req: Request, res: Response) => {
  const requestId = req.headers['x-request-id'] as string ?? 'unknown';
 ...
 *         description: Unauthorized - Invalid or missing token
 */
router.post('/extract-skills', authMiddleware, requireKyc, async (req: Request, res: Response) => {
  const requestId = req.headers['x-request-id'] as string ?? 'unknown';
 ...
 *         description: Freelancer profile not found
 */
router.get('/skill-gaps', authMiddleware, requireKyc, async (req: Request, res: Response) => {
  const authReq = req as AuthenticatedRequest;
  const requestId = req.headers['x-request-id'] as string ?? 'unknown';
  const userId = authReq.user.userId;
