...
async function main() {
  ...
  const escrow = new ethers.Contract(deployment.contracts.FreelanceEscrow, escrowABI, provider);
  const reputation = new ethers.Contract(deployment.contracts.FreelanceReputation, reputationABI, provider);
